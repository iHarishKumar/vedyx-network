...
contract VoterRewardsTest is Test {
    ...
    function test_RewardDistribution_SingleCorrectVoter() public {
        ...
        uint256 expectedPenalty = (user2StakeBefore * PENALTY_PERCENTAGE) /
            10000; // 30 ether
        uint256 finalizationFee = (expectedPenalty *
            FINALIZATION_FEE_PERCENTAGE) / 10000; // 1% of penalty
        ...
        VedyxTypes.Staker memory stakeUser1After = votingContract.getStakerInfo(
            user1
        );
        VedyxTypes.Staker memory stakeUser2After = votingContract.getStakerInfo(
            user2
        );
        VedyxTypes.Staker memory stakeUser3After = votingContract.getStakerInfo(
            user3
        );

        assertEq(stakeUser1After.stakedAmount, user1StakeBefore + user1Reward);
        assertEq(
            stakeUser2After.stakedAmount,
            user2StakeBefore - expectedPenalty
        );
        assertEq(stakeUser3After.stakedAmount, user3StakeBefore + user3Reward);
    }

    function test_RewardDistribution_MultipleCorrectVotersProportional()
        public
    {
        vm.prank(user1);
        ...
        uint256 finalizationFee = (totalPenalty * FINALIZATION_FEE_PERCENTAGE) /
            10000; // 1% of penalty
        ...
        uint256 expectedUser1Reward = (penaltiesForDistribution * 600 ether) /
            totalCorrectVotingPower; // 29.7 ether
        uint256 expectedUser2Reward = (penaltiesForDistribution * 400 ether) /
            totalCorrectVotingPower; // 19.8 ether
        ...
        assertEq(
            stakeUser1After.stakedAmount,
            user1StakeBefore + expectedUser1Reward
        );
        assertEq(
            stakeUser2After.stakedAmount,
            user2StakeBefore + expectedUser2Reward
        );
    ...
    function test_RewardDistribution_MultipleIncorrectVoters() public {
        ...
        uint256 finalizationFee = (totalPenalties *
            FINALIZATION_FEE_PERCENTAGE) / 10000; // 1% of penalties
        ...
        assertEq(
            stakeUser1After.stakedAmount,
            user1StakeBefore + rewardAfterFee
        );
    ...
    function test_RewardDistribution_AllVotersIncorrect() public {
        ...
        assertEq(
            stakeUser3After.stakedAmount,
            user3StakeBefore + rewardAfterFee
        );
    }
    ...
    function test_RewardDistribution_WithKarmaBonus() public {
        ...
        VedyxTypes.Staker memory stakeUser1 = votingContract.getStakerInfo(
            user1
        );
        ...
        (, bool user1VotedFor, uint256 user1VotingPower) = votingContract
            .getVote(votingId2, user1);
        (, bool user2VotedFor, uint256 user2VotingPower) = votingContract
            .getVote(votingId2, user2);
        ...
        VedyxTypes.Staker memory stakeUser2 = votingContract.getStakerInfo(
            user2
        );
        assertEq(stakeUser1.karmaPoints, 10);
        ...
        VedyxTypes.Staker memory stakeUser1Before = votingContract
            .getStakerInfo(user1);
        VedyxTypes.Staker memory stakeUser2Before = votingContract
            .getStakerInfo(user2);
    ...
    function test_RewardDistribution_FourVotersMixedOutcome() public {
        ...
        uint256 expectedUser1Reward = (penaltiesForDistribution * 1000 ether) /
            totalCorrectVotingPower;
        uint256 expectedUser2Reward = (penaltiesForDistribution * 600 ether) /
            totalCorrectVotingPower;
        ...
        VedyxTypes.Staker memory stakeUser4After = votingContract.getStakerInfo(
            user4
        );
        ...
        uint256 totalStakesAfter = stakeUser1After.stakedAmount +
            stakeUser2After.stakedAmount +
            stakeUser3After.stakedAmount +
            stakeUser4After.stakedAmount;
        uint256 originalTotal = user1StakeBefore +
            user2StakeBefore +
            400 ether +
            200 ether;
    ...
    function test_RewardDistribution_PenaltyExceedsStake() public {
        ...
        uint256 finalizationFee = (actualPenalty *
            FINALIZATION_FEE_PERCENTAGE) / 10000; // 1% of penalty
        ...
        assertEq(
            stakeUser2After.stakedAmount,
            user2StakeBefore - actualPenalty
        );
        assertEq(stakeUser3After.stakedAmount, user3StakeBefore + user3Reward);
    }
}
